...
- the settings drawer uses stable sidebar navigation for `Dashboard`, `Feeds`, `Profile`, `Model`, and `App`, with a horizontal fallback on narrow screens: Dashboard prioritizes active jobs and progressively discloses targeted sync, reclassification, usage, and diagnostics; Feeds edits an isolated local draft and opens catalog/custom additions as a focused secondary view; Profile keeps one primary review document and a secondary feedback queue; Model keeps credentials and defaults visible while tuning and pricing stay under Advanced with one save action; App owns Zotero connection and collection selection, scheduling, local app fields, and compact diagnostics
