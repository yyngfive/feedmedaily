...
The profile carries a live topic registry (`topic_taxonomy`, `{id, label}` entries with system-assigned immutable ids) and direct/indirect rules may carry topic id tags; unrelated rules and scope never carry topics. The classifier assigns each related paper at most one topic in the same call: it picks the best-fitting id among the union of topic tags on matched rules, and returns no topic when no tagged rule fits. Stored classifications keep the assignment in the existing `topic_tags_json` column with three states: an empty array means "no topic decision recorded" (legacy rows and unrelated papers), the reserved sentinel `none` means "processed but nothing fits" (also written when the model returns an unknown id, as an anti-hallucination guard), and a real id is the assigned topic. Labels resolve for display through the current registry, so renaming a topic is a pure display change; deleting a topic leaves orphan ids that render as unassigned until a reclassification.
