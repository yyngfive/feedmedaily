...
Database cleanup is a separate cancellable pipeline job exposed through `/api/admin/cleanup`. It snapshots the SQLite file with `VACUUM INTO` before mutation, builds exact duplicate components from normalized URL/DOI keys, retains a classified or oldest canonical row, moves feedback/Zotero references, and deletes only unclassified duplicate rows when the normalized titles agree; the retained row takes over the deleted copy's DOI — and adopts the DOI-based `paper_key` when that key is free — only when the deleted copy's DOI arrived with its feed item, which its stored `doi:` key proves, so the next fetch resolves to the surviving row instead of recreating the duplicate. A DOI that enrichment guessed later (`url:`/`title:` key) is left behind, because the feed entry for that article carries no DOI and the next fetch reaches the retained row by its URL key anyway. Duplicate pairs where both rows are already classified are left untouched by design. Duplicate entries that both have DOI values but still have non-matching normalized titles become a DOI-conflict review instead of an automatic deletion; title variants such as `Inside Back Cover:` remain ordinary duplicate reviews. A standalone DOI is cleared and re-keyed to its publisher URL only when Crossref or OpenAlex provides an explicit title/date mismatch; provider failures and incomplete records are persisted in `cleanup_reviews` and are never auto-deleted. Review decisions (`keep`, `delete`, `delete_match`, `clear_doi`, or `clear_match_doi`) are audit-stamped and applied through the same serialized pipeline; title-duplicate reviews let the user choose either side to delete, while DOI-conflict reviews only let the user clear one side's DOI or keep both entries unchanged. The retained unclassified article remains queued for the next cleanup batch. Title-duplicate reviews are keyed per unordered paper pair, so one keep or delete decision covers both scan directions instead of creating a mirrored review for the other paper. Deleting a paper closes every pending review that references it as candidate or matched, and startup repairs legacy pending reviews whose matched paper no longer exists; both prevent a review with no actionable decision from pausing bulk classification forever. Bulk classification pauses whenever any pending review remains and resumes only on a later cleanup run after review completion. Pre-cleanup `VACUUM INTO` snapshots are pruned to the newest ten after each successful backup. Legacy deferred rows migrate to pending.
...
- the backend judges a verifier window that exits early by its delivery state, not by how fast it returned: a warm `verification-profiles` host profile finishes the capture in ~500 ms without human input, and that fast exit is a success as long as the callback delivered XML; captured XML is kept even when the verification path also reports a warning, so a protected feed cannot silently lose its window of papers
...
- classifier models: paper relevance classification (`deepseek-flash`, `glm-5.3-flash`, `qwen3.8-flash`, `mimo-v2.5`)
- profile model: onboarding profile generation and feedback-driven profile revision (`deepseek-v4-pro` by default, plus `glm-5.3`, `qwen3.8-max-0902`, `mimo-v2.5-pro`)

The classifier catalog owns provider endpoints and supported thinking contracts. DeepSeek retired the `deepseek-v4-flash` call name and serves DeepSeek-V4.1-Flash behind `deepseek-flash`; retired DeepSeek Flash names stay readable in saved selections, `.env` files, and structured updates, and resolve to that same catalog entry instead of being dropped or migrated to another provider. The global `SCIRSS_CLASSIFIER_THINKING` preference is exposed under Model → Advanced and can only toggle the lowest supported level: DeepSeek V4.1 Flash and Qwen3.8-Flash use `low`, MiMo-V2.5 uses the Chat API's lowest expressible `enabled` mode, and Zhipu GLM-5.3-Flash always remains `enabled` with `reasoning_effort=low` and deterministic sampling even when the preference is disabled. Disabled DeepSeek sends `thinking.type=disabled`; disabled Qwen sends `reasoning_effort=none`; disabled MiMo sends `thinking.type=disabled` with its documented `max_completion_tokens` field. DeepSeek and MiMo requests use a 4096 completion-token floor while thinking is enabled, without lowering a larger batch-derived limit. The shared adapter is used by batch classification, single-paper degradation, title translation, and connection tests; title translation remains forced to non-thinking mode. Managed providers never enter the generic disabled-thinking fallback, and no request automatically switches providers.
...
- profile models reuse the same per-provider classifier keys; the legacy DeepSeek-only `SCIRSS_PROFILE_API_KEY` / `SCIRSS_PROFILE_BASE_URL` / `SCIRSS_PROFILE_MODEL` remain honored as a migration fallback, and an unknown custom legacy provider keeps running unmanaged

The profile catalog mirrors the classifier's resolution: `SCIRSS_PROFILE_DEFAULT_MODEL` selects among the fixed entries, each entry's `configured` state comes from the shared provider key, and the selected entry feeds the flat profile settings consumed by `internal/profile`. The profile request builder applies per-provider thinking controls: DeepSeek pins `reasoning_effort=low` with a 16384 output floor (even low effort measured over 8192 reasoning tokens on the proposal task), Qwen maps the preference to `reasoning_effort` (`low`/`none`) with an 8192 floor, MiMo — which exposes no effort level — uses `thinking.type` plus `max_completion_tokens` with a 16384 floor because its default-depth reasoning alone exceeds 8192, and Zhipu GLM-5.3 — an always-thinking model — pins `reasoning_effort=low` with an 8192 floor but never receives a disabled toggle. Thinking-enabled profile calls use a 300-second HTTP timeout because several providers stream for minutes; thinking-disabled calls keep the historic 60-second budget. Provider responses log `finish_reason` and reasoning-token counts so thinking-budget truncation is diagnosable, and topic labels that models illegally attach to unrelated or scope changes are stripped instead of failing the proposal. A `POST /api/settings/profile-models/test` job verifies a catalog entry through the same request path. Onboarding's model step reuses the same classifier and profile editors, so one provider key configures both roles and the initial profile runs on the selected default profile model.
...
The current classification path stores relevance, confidence, reason, recommended action, and translated title. Recommended action remains part of the local API/database compatibility shape but is derived deterministically by the application (`direct -> read`, `indirect -> scan`, `unrelated -> skip`) rather than generated by the model. Classifier requests use bounded retry handling for transient provider failures and malformed JSON responses; if a batch still fails, sync and reclassify jobs fall back to single-paper classification so successful papers can still be persisted.

### Topics

The profile carries a live topic registry (`topic_taxonomy`, `{id, label}` entries with system-assigned immutable ids) and each direct/indirect rule carries at most one topic id tag (legacy multi-tag entries deterministically truncate to the first); unrelated rules and scope never carry topics. The classifier assigns each related paper at most one topic in the same call: it picks the best-fitting id among the topics of matched rules, and returns no topic when no tagged rule fits. Stored classifications keep the assignment in the existing `topic_tags_json` column with three states: an empty array means "no topic decision recorded" (legacy rows and unrelated papers), the reserved sentinel `none` means "processed but nothing fits" (also written when the model returns an unknown id, as an anti-hallucination guard), and a real id is the assigned topic. Labels resolve for display through the current registry, so renaming a topic is a pure display change; deleting a topic leaves orphan ids that render as unassigned until a reclassification.

Profile editing matches topic drafts by stable local keys, including entries without persisted ids. Deletion removes only that topic's rule references. Proposal rewrites/merges preserve source tags when `topics_after` is omitted or null; an explicit empty array clears them. Merging differently tagged rules without an explicit topic decision is rejected. Deleted feedback topics reopen and submit as `No topic`.

Existing related papers are backfilled through the admin reclassify scope `topics`, a topic-only pass that keeps the stored relevance decision and appends a new classification row with only the topic result; it reuses the cancellable background job machinery and is always manually triggered.

Topic feedback is part of `Mark wrong`: the feedback modal offers the registry topics plus "no topic" and can create a new registry entry inline through `POST /api/profile/topics` (label-idempotent, bumps the profile version). Feedback rows store `original_topic`/`corrected_topic` next to the relevance columns, and the profile model may respond to topic feedback with rule tag changes (`topics_before/topics_after` on rule changes) and add-only topic changes in a proposal; it never merges, splits, or deletes topics. Applying a proposal resolves topic labels to registry ids (creating ids for accepted topic additions) and fails when an accepted rule change references a label no accepted topic change provides.

`/api/report/latest` carries a `topics` block with the registry snapshot, per-topic counts, an `unassigned` count (sentinel plus orphan ids), and an `unprocessed` count (related papers never put through a topic decision), so the review workspace can filter and render topics without a separate profile request.

Classifier defaults are DeepSeek V4.1 Flash (`deepseek-flash`) with thinking disabled and batch size `5`. Advanced settings can enable only the lowest provider reasoning level; GLM cannot be disabled. Managed providers and the legacy generic path retain bounded same-model retries. Each sync/reclassify/model-test job captures its resolved model, key, effective provider controls, batch settings, and thinking token floor at queue time, so changing the default or thinking preference affects only later jobs; the provider pricing remains built in and fixed.
