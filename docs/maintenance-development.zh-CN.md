...
- 模型计价：DeepSeek、GLM、Qwen 和 MiMo 的费率由 `internal/llmusage` 内置，按 provider 和响应时间选择；不从 Settings、`.env` 或系统环境变量读取，也不允许用户修改。已有配置中的旧价格键会在启动时清理；usage ledger 仍保存每条历史记录实际采用的费率快照
...
3. 更新前端 `web/src/api/client.ts`、`web/src/shared/types.ts` 和共享的 `ClassifierModelsEditor`。
4. 更新 `docs/api.zh-CN.md`。
5. 添加或调整 `internal/api/*_test.go`。
6. 运行 `go test ./...`，`corepack pnpm --dir web test`，如果前端类型受影响再运行 `corepack pnpm --dir web build`。
...
6. 新增 LLM 调用时显式传递 job collector，并确保成功响应只记录一次且带响应时间；collector 使用 `internal/llmusage` 的内置固定费率。DeepSeek 高峰价只适用于北京时间周一至周五 9:00–12:00、14:00–18:00，周末和其余时段使用空闲价；已完成 ledger 保存具体费率快照，后续内置费率变更不得回算历史。usage ledger 写入失败只能记录 warning，不能反向让业务 job 失败。
